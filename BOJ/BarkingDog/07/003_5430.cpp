	...
	while (t--) {
		...
		for (char c : p) {
			if (c == 'R') {
				...
				if (ret_str.empty()) {
					flag = true; //배열에 값이 없는데 D가 호출될 경우
		...
		if (flag) { //배열에 값이 없는데 D가 호출될 경우
			cout << "error" << '\n';
		}
		else { //배열에 값이 없는데 R이 호출될 경우 + 값이 있는 경우에 D, R이 호출될 경우
			if (ret_str.empty()) { //1. 배열에 값이 없는데 R이 호출될 경우
				cout << "[]" << '\n';
			}
			else { //2, 3. 배열의 값이 남아있을 때의 D, R 연산 결과 출력
				cout << '[';
