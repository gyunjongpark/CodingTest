...
struct Customer {
	int start; //출발지점의 y좌표 * 100 + x좌표
	int end; //도착지점의 y좌표 * 100 + x좌표
};
Customer customer[400]; //20 x 20이므로 최대 400명의 승객
...
int find_shortest_customer() {
	...
	int candi[400] = { 0, };
	int candi_idx = 0; //택시로부터 가까운 거리에 있는 손님 발견 시, 해당 인덱스를 손님 번호에 부여
	int candi_distance = -1;

	while (!q.empty()) { //bfs를 통해 최단거리의 승객을 찾아낸다
		Taxi cur = q.front(); q.pop();

		//갱신되었는데 기존의 거리보다 멀다면 break
		if (candi_distance != -1 && cur.distance > candi_distance) break;

		//택시 좌표부터 탐색(택시인 동시에 승객이 있을 가능성)
		int y = cur.pos / 100;
		int x = cur.pos % 100;

		if (board[y][x] >= 0) { //승객이라면
			candi[candi_idx] = board[y][x]; //승객의 번호를 candi 목록에 저장
			candi_idx++; //인덱스 증가(승객의 번호와 인덱스는 다른 개념이다)

			candi_distance = cur.distance; //최소 거리 갱신(bfs의 특성)
		}
	...
	//우선순위를 적용해 손님을 선택한다
	int ret = -1; //승객 인덱스
	int candi_val = 10000; //행번호 * 100 + 열번호 연산과 비교. 배열은 최대 2020

	for (int i = 0; i < candi_idx; i++) {
		if (candi_val > customer[candi[i]].start) {
			candi_val = customer[candi[i]].start; //행번호, 열번호가 작은 손님 선택
			ret = candi[i]; //가장 우선순위의 승객 번호 저장
		}
	}
